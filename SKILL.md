...
优先产出可直接给业务、政府事务、法务合规或经营团队使用的清单，不写成泛泛新闻摘要。
...
如果事项明显依赖地方口径（如经信局、教育局、综合执法局、投促局、资规局、住建局、生态环保局等），先给全国/部委层面结果，并明确说明：`完整地方版需指定省/市。`
...
- 同一部门在近 12 个月内如存在多项政策修改、修订、优化、废止、替代或新增实施细则，必须**全部逐条统计并分别入表**。
- 不得只选代表性样本，不得抽样，不得将多个文件合并成一条笼统概述。
- 同一部门的多条记录默认按发布日期倒序排列。
- 每条记录必须对应一份已核实的官方原文，并提供真实政策文件编号或明确写明“无文号（原网页未列明）”。
